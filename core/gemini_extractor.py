...
def _build_prompt(titles: list, grade: int, count: int) -> str:
    ...
    detail_guide = {
        1: ('세부 장소(동·역·구 포함) + 상호명. 상호명 없으면 세부 장소+구체 항목. '
            '전국 브랜드면 브랜드+매장명+구체 세부항목. 최대 4어절. 탐색 의도어 없이.'),
        2: ('세부 장소+구체 서비스/항목(상호명 제외). '
            '전국 브랜드면 브랜드+매장 타입+구체항목. 3~4어절. 탐색 의도어 없이.'),
        3: ('넓은 장소+상호명(지역 상호인 경우). 상호명 없으면 넓은 장소+구체항목/특성. '
            '전국 브랜드면 브랜드+구체 메뉴/항목(매장명 제외). 2~3어절. 탐색 의도어 없이.'),
    ...
    if grade == 5:
        ...
        brand_rule = ('- 제목에 지역 상호명(가게 이름)이 있으면 "넓은 장소 + 상호명" 형태로 구성해.\n'
                      '- 전국 체인·글로벌 브랜드면 브랜드명 + 구체 메뉴/항목(매장명·지점명 제외).\n'
                      '- 탐색 의도어(맛집·추천·카페 등) 없이.')
    elif grade == 2:
        brand_rule = ('- 상호명은 제외해. 세부 장소 + 구체 서비스/항목 위주로 구성해.\n'
                      '- 전국 브랜드면 브랜드명 + 매장 타입 + 구체항목. 탐색 의도어 없이.')
    else:
        brand_rule = ('- 제목에 지역 상호명이 있으면 "세부 장소 + 상호명" 형태로 구성해.\n'
                      '- 전국 브랜드면 브랜드명 + 매장명 + 구체 세부항목. 최대 4어절.')

    if grade == 1:
        inference_rule = (
            '- 제목에 있는 단어들을 조합해 구체적인 검색어를 만들어.\n'
            '- 제목에 검색 가능한 명사가 없으면 있는 단어만 사용해. 없는 단어를 생성하지 마.\n'
            '- 맛집·여행·스파·후기 같은 탐색 의도어는 포함하지 마.'
            ' 장소+핵심특징+카테고리+브랜드명 조합으로만 구성해.\n'
            '- 상호명 뒤에 지역명·객실명 등 다른 단어를 붙여 쓰지 마. 상호명은 그대로 단독으로 써.'
        )
    elif grade == 2:
        inference_rule = (
            ...
            '- 제목에 검색 가능한 명사가 없으면 keywords를 빈 배열 []로 반환해.'
        )
    else:
        inference_rule = (
