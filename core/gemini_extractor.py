...
def _build_prompt(titles: list, grade: int, count: int) -> str:
    ...
    detail_guide = {
        ...
        3: ('카테고리 2개 조합 또는 카테고리+구체 특성(재료·사양·조리법 등). '
            '지역 상호명 제외. 탐색 의도어·형용사 없이. 2~3어절. '
            '예: 이삭토스트 햄 스페셜 토스트, 광안리 술집 안주, 암막 우양산.'),
        4: ('세부 장소(동·역·구 등)로 교체하거나 탐색 의도어(맛집·카페·이용·구매처 등)를 추가. '
            '세부 장소나 탐색 의도어가 적용되기 어려우면 카테고리+구체 특성 조합으로 추출해. '
            '전국 브랜드면 브랜드+매장 타입+핵심 기능. 상호명 제외. 2~3어절. '
            '제목에서 카테고리를 파악할 수 없으면 keywords를 빈 배열 []로 반환해.'),
        5: ('가장 대표적인 장소+핵심 카테고리 또는 전국 브랜드+핵심 카테고리. '
            '지역 상호명·매장명 제외. 탐색 의도어 없이. 1~2어절. '
            '예: 광안리 술집, 이삭토스트 메뉴, 크로커다일 양산.'),
    }.get(grade, '중간 키워드')
    ...
    if grade == 5:  # UI 1등급 (대표)
        brand_rule = ('- 지역 상호명·매장명(맹구포차·메리윤 등)은 제외해.\n'
                      '- 전국 브랜드(이삭토스트·크로커다일·디어오늘 등)는 핵심 카테고리와 함께 포함 가능.')
    elif grade == 4:  # UI 2등급
        brand_rule = ('- 지역 상호명은 제외해. '
                      '전국 체인 브랜드의 매장 타입(리저브·올데이·딜리버리 등)은 세부 위치처럼 포함 가능.')
    elif grade == 3:  # UI 3등급
        brand_rule = ('- 지역 상호명·매장명(맹구포차·메리윤 등 지역 가게 이름)은 제외해. 전국 브랜드(이삭토스트·크로커다일 등)는 포함 가능.\n'
                      '- 형용사(예쁜·맛있는·최애·채썬 등)는 키워드에 포함하지 마.')
    elif grade == 2:  # UI 4등급
        ...
        brand_rule = ('- 상호명이 있으면 반드시 포함. UI 4등급에서 사용하지 않은 다른 카테고리 또는 구체항목과 상호명을 조합해.\n'
                      '  (예: 해운대 브런치 메리윤, 부산역 자이나타운 장춘향, 양산 중부동 사계절옷수선)\n'
                      '- 전국 브랜드면 브랜드명+핵심 카테고리+부가 특성 형태 (3어절).\n'
                      '  (예: 디어오늘 바디로션 보습감, 한의비책 관절뜸패치 무릎통증완화)\n'
                      '- 상호명이 제목에 없으면 카테고리+구체 특성 조합으로 가장 세부적인 키워드를 추출해.')

    if grade == 1:  # UI 5등급 (세부)
        inference_rule = (
            '- 제목에 있는 단어들을 조합해 가장 구체적인 검색어를 만들어.\n'
            '- 상호명이 없으면 카테고리+구체 특성 조합으로 추출해. 빈 배열 []로 반환하지 마.\n'
    ...
    if grade in (1, 2, 3):  # UI 5·4·3등급 — 상호명 포함 가능 등급
        brand_min_rule = (
            f'- [필수] 키워드에 상호명·브랜드명을 직접 포함시킬 때만 적용:\n'
            f'  "브랜드+대표카테고리" 2어절 단독 조합은 금지. 구체 단어를 추가해 3어절 이상으로 구성해.\n'
            f'  금지 예: 크로커다일 양산, 이삭토스트 메뉴\n'
            f'  허용 예: 크로커다일 암막 우양산, 이삭토스트 햄 스페셜 토스트\n'
            f'  (키워드에 브랜드가 없으면 이 규칙은 적용하지 않아도 됨)\n'
        )
    else:
        brand_min_rule = ''
