...
def _build_prompt(titles: list, grade: int, count: int) -> str:
    ...
    detail_guide = {
        ...
        3: ('카테고리 2개 조합 또는 카테고리+구체 특성(재료·사양·조리법 등). '
            '상호명·브랜드명 전부 제외. 탐색 의도어·형용사 없이. 2~3어절. '
            '예: 광안리 술집 안주, 애호박전 당근, 암막 우양산.'),
    ...
    if grade == 5:  # UI 1등급 (대표)
        ...
        brand_rule = ('- 상호명·매장명·전국 브랜드명(이삭토스트·스타벅스 등) 모두 제외해. 브랜드가 아닌 카테고리·재료·특성 단어만 사용.\n'
                      '- 형용사(예쁜·맛있는·최애·채썬 등)도 키워드에 포함하지 마.')
    elif grade == 2:  # UI 4등급
