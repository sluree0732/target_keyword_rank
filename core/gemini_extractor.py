...
def _build_prompt(titles: list, grade: int, count: int) -> str:
    ...
    grade_spec = {
        5: (  # UI 1등급 (대표)
            '반드시 1~3어절. 4어절 이상 절대 금지.\n'
            '  형태: 장소+카테고리 또는 서비스·매장 브랜드+카테고리.\n'
            '  예: 광안리 술집 / 이삭토스트 메뉴 / 크로커다일 양산 / 여성청결제 / 소고기 뭇국\n'
            '  브랜드 포함 기준:\n'
        ...
        4: (  # UI 2등급
            ...
            '  탐색 의도어 선택 기준 (가장 적합한 단어 하나만 선택):\n'
            '    레시피·요리·집밥·만들기 제목 → 만들기, 레시피, 방법 中 하나  ← 추천 금지\n'
            '    식당·카페·먹방 탐방 제목    → 맛집, 먹는곳 中 하나            ← 추천 금지\n'
            '    제품·효능·성분 정보 제목    → 사용, 사용법, 방법 中 하나       ← 추천 금지\n'
            '    구매·가격·쇼핑 제목         → 구매, 구매처\n'
            '    여행·나들이·드라이브 제목   → 코스, 가볼만한곳 中 하나         ← 추천 금지\n'
            '    제목에 추천·순위·best 포함  → 추천 허용\n'
            '  지역 상호명·제품 브랜드는 제외.'
        ),
        3: (  # UI 3등급
            '2~3어절. 카테고리+구체 특성 조합.\n'
            '  형태: 재료+카테고리 / 카테고리+콘텐츠유형 / 카테고리+성분·사양\n'
            '  예: 소고기 뭇국 / 광안리 술집 안주 / 소고기 뭇국 레시피\n'
        ...
        2: (  # UI 4등급
            ...
            '  형태 ③: 제품 브랜드+카테고리\n'
            '  예: 해운대 파스타 메리윤 / 노스페이스 경량 패딩 / 포마이도터 여성청결제\n'
